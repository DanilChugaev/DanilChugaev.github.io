			...
			*/
			...
			return {
				...
				"SwitchStatement:exit"(node) {
					...
					for (const post of funcInfo.currentSegments) for (const prev of post.prevSegments) {
						if (isEffectivelyUnreachable(prev)) continue;
						if (caseBodySegments.has(prev)) return;
					}
	...
	*/
	function isGlobalOrImportedFromVue(callee, context) {
		...
		if (variable !== null && variable.defs.length === 1) {
			...
			if (def.type === "CatchClause" || def.type === "ClassName" || def.type === "FunctionName" || def.type === "TDZ") return false;
		}
		return true;
	}
	/**
