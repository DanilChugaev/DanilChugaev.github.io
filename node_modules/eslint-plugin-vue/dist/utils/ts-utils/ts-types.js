...
var require_ts_types = /* @__PURE__ */ require_runtime.__commonJSMin(((exports, module) => {
	...
	*/
	function isExhaustiveSwitch(context, switchNode) {
		...
		const caseHasUndefined = [...caseTypes].some((type) => (type.flags & ts.TypeFlags.Undefined) !== 0);
		for (const member of unionConstituents(discriminantType)) if (!intersectionConstituents(member).some((part) => isTypeLiteralLike(part, ts) && (caseTypes.has(part) || caseHasUndefined && (part.flags & ts.TypeFlags.Undefined) !== 0))) return false;
		return true;
	}
}));
