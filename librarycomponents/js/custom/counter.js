...
CountdownTimer.prototype={
    ...
    countDown:function(){
        ...
        if( ( this.tl - today ) > 0 ){
            timer += '<span class="number-wrapper"><div class="line"></div><span class="number day">'+day+'</span></span>';
...
function CDT(){
    var className = document.getElementsByClassName("counter-reset"),
        arr = [].slice.call(className),
        time;
    ...
    arr.forEach(function (item, i, arr) {
        ...
        // Set countdown limit
        var tl = new Date(time);

        // You can add time's up message here
        var timer = new CountdownTimer(id,tl,'<span class="number-wrapper"><div class="line"></div><span class="number end">Время вышло!</span></span>');
        timer.countDown();
    });
